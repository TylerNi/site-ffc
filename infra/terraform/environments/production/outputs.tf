...
# À reporter tel quel dans les VARIABLES de l'environnement GitHub `production`
# (`terraform output -json github_environment_variables`).
output "github_environment_variables" {
  description = "Variables à définir dans l'environnement GitHub production."
  value = {
    AWS_DEPLOY_ROLE_ARN = data.terraform_remote_state.global.outputs.github_deploy_role_arns["production"]
